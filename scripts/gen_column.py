...
def save_image_prompts(col_id: str, image_prompts: list, date_folder: str) -> str | None:
    """画像プロンプトJSONをSupabase Storageに保存してパスを返す"""
...
def run(dry_run=False, theme_index=None):
    ...
    if save_to_supabase(col):
        ...
        if image_prompts:
            ...
            print(f"[gen] 画像プロンプトJSON保存: {prompts_path.name} ({len(image_prompts)}件)")
        else:
            print(f"[gen] ⚠️ image_promptsが空です")
